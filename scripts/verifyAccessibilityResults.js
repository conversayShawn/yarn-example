...
getAccessibilityResults({
  ...
  .then((results) => {
    ...
    console.log(
      `Received ${
        summary.isPartialReport ? "partial" : ""
      } results for run #${runNumber}.`
    );
    ...
    if (total > 0) {
      ...
      console.warn(
        `${total} Accessibility violations detected. Please review and address these issues.`
      );
    } else {
    ...
    axios
      .post("https://webhook.site/64d67fc7-3bf0-4db4-83e8-f36d95373874", {
        runNumber,
  ...
  .catch((error) => {
    console.error(
      "An error occurred while fetching the accessibility results:",
      error
    );
    process.exit(1);
  });
